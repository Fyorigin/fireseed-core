...
## File Integrity & Structural Signing / 文件完整性与结构签署声明

This repository is under **automated file integrity surveillance**.  
All file modifications are tracked via cryptographic hash values (SHA-256) and UTC timestamps, recorded in [`hash_manifest.json`](./hash_manifest.json) automatically by GitHub Actions.
[View Provenance Statement →](./PROVENANCE.md)

本仓库受**自动化文件完整性监控机制**保护。  
所有文件修改均由 GitHub Actions 自动记录，并以 SHA-256 哈希值与 UTC 时间戳形式登记于 [`hash_manifest.json`](./hash_manifest.json)。

**Do not edit or remove files without authorized protocol compliance.**  
Any unauthorized changes will be logged, and structural trust will be invalidated.

**请勿擅自编辑或删除文件，任何未授权更改将被记录，并导致结构信任失效。**

This system is governed by the [Fireseed-OEP-1.0](./LICENSE.md) protocol, and integrated with signed declarations within each file where applicable.
